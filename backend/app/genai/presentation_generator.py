...
class MenuPresentationGenerator:
    ...
    def generate(
        ...
        *,
        ...
        locale: str,
        _target_locale: str | None = None,
        _country_aware: bool = False,
        on_provider_attempt: (
        ...
        target_locale = normalize_presentation_locale(_target_locale or locale)
        request: dict[str, Any] = {
            "instructions": (
                self._country_aware_instructions(target_locale)
                if _country_aware
                else self._instructions(locale)
            ),
            "input": [
                {
                    "role": "user",
        ...
        if self.provider.capabilities.structured_output:
            request["text"] = {
                "format": {
                    "type": "json_schema",
                    "name": (
                        "yobi_country_aware_menu_presentation_v1"
                        if _country_aware
                        else "yobi_grounded_menu_presentation_v7"
                    ),
        ...
        try:
            ...
            for generated in result.items:
                ...
                for field_name in (
                    ...
                    invalid_script = bool(value) and not text_uses_target_script(
                ...
                if generated.personalization_applied:
                    required_yobi_fields.add(
                        "presentation_country_context"
                        if _country_aware
                        else "country_preference"
                    )
    ...
    def generate_country_aware(
        ...
        *,
        ...
        target_locale = normalize_presentation_locale(locale)
        return self.generate(
            items=items,
            locale=presentation_locale_label(target_locale),
            _target_locale=target_locale,
            _country_aware=True,
...
""".strip()
    ...
    def _country_aware_instructions(self, locale: str) -> str:
        language_label = presentation_locale_label(locale)
        return f"""
You are YOBI's grounded menu explainer. Another model already selected the menus. Return exactly
one JSON object matching the supplied schema, written in {language_label}, with every requested
menu_id exactly once. Never add, remove, reorder, rank, or replace a menu.
...
4. wiki_passages explain the general food concept only. They never prove this restaurant's exact
   recipe, ingredients, certification, popularity, availability, or preparation.
5. presentation_country_context is a server-owned comparison cue, not evidence about a nation's
   people or about menu popularity.

Field rules:
- localized_subtitle is one concise phrase in {language_label} that helps a visitor understand the
  exact listing. It may equal the server title when no supported clarification is available.
- localized_source_description is a faithful, natural translation of source_description_ko into
  {language_label}, not a summary, country adaptation, or phonetic transliteration. If the source
...
- A spice comparison is allowed only when presentation_country_context.comparison_is_complete is
  true. Use its representative_dish_en only as the named demo spice reference and preserve the
  LESS/SIMILAR/MORE relationship. Put at most one such comparison in yobi_long_explanation. If the
  user country differs from spice_reference_country_code, call it the visitor's selected spice
  reference, not the visitor's country.
- Set personalization_applied=true only if country context is visibly used in the explanation.
- review_summary is the only review field. Base it exclusively on synthetic_reviews. If none are
  supplied, state neutrally that no review summary is available.
- For a compound listing, explain each supplied menu_component without transferring one
  component's ingredient, temperature, texture, or cooking method to another. Return its supplied
  component_id once in covered_component_ids and copy a matching visible phrase into
  component_mentions.
- Do not return localized_title, evidence/source arrays, or option-localization arrays. The server
  owns title, selection, order, and provenance.

Never stereotype a nationality or invent an analogy, ingredient, taste, cooking method,
certification, dietary safety, popularity, order count, restaurant practice, or availability. Do
not expose internal IDs in prose. Do not emit Markdown, analysis, or a preamble.

Before returning, silently verify: all menu IDs are present once; every field uses
{language_label}; restaurant-source digits/units/Latin tokens are unchanged; country comparison is
grounded in the supplied context; and no unsupported country claim was added.
Prompt version:
