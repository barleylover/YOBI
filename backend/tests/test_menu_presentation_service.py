...
def _presentation(
    ...
    *,
    ...
    return MerchantMenuPresentation(
        ...
        evidence_map={
            "wiki_passages": [
                {"evidence_id": "wiki-1", "evidence_type": "WIKI_PASSAGE", "content": "Rice cakes are chewy."},
                {"evidence_id": "wiki-2", "evidence_type": "WIKI_PASSAGE", "content": "The dish is served with sauce."},
            ],
            "menu_facts": [],
            "synthetic_reviews": [
                {"review_id": "review-1", "topic": "TEXTURE", "rating": 5, "review_text": "Chewy texture."},
                {"review_id": "review-2", "topic": "PORTION", "rating": 4, "review_text": "Generous portion."},
            ],
        ...
        "items": [
            {
                ...
                "used_source_fields": ["wiki_passages", "synthetic_reviews"],
...
def test_invalid_grounding_contract_keeps_deterministic_copy_without_fallback() -> None:
    ...
    assert provider.calls == ["xai.grok-4.3"]
    assert page.items[0].model_dump(mode="json") == original.model_dump(mode="json")
...
def test_nakji_kimchi_bibimbap_fallback_subtitle_explains_nakji() -> None:
    assert deterministic_localized_subtitle(
        "en",
        title_ko="낙지김치비빔밥",
        localized_title="Nakji Kimchi Bibimbap",
    ) == "Octopus and kimchi mixed rice bowl"
    assert "タコ" in deterministic_localized_subtitle(
        "ja",
        title_ko="낙지김치비빔밥",
        localized_title="ナクチキムチビビンバ",
    )
...
def test_partial_cache_hit_batches_only_missing_presentations() -> None:
    ...
    repository = PresentationRepository(
        MerchantMenuPresentationPage(items=[first, second])
    )
    provider = PresentationProvider()
    service = MenuPresentationService(
        repository,  # type: ignore[arg-type]
