...
`2026-08-06`. Oracle deployment can re-embed the authored chunks with the configured
embedding provider before activation. Source/document hashes and embedding metadata
...
output, streaming capability, timeout, and bounded retry/backoff. Dedicated tests use
contract fixtures; the existence of that adapter is not evidence that a paid
dedicated endpoint has been provisioned.
...
`GROUNDING_REJECTED`, or `PROVIDER_UNAVAILABLE` (with a final unknown-provider
