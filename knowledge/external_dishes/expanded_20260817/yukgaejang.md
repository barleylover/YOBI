---
{
  "concept_id": "dish_yukgaejang",
  "concept_type": "FAMILY",
  "name_ko": "육개장",
  "name_en": "Yukgaejang",
  "aliases": [
    "육개장",
    "spicy Korean beef soup"
  ],
  "language": "en",
  "parents": [
    {
      "concept_id": "dish_korean_cuisine",
      "relation_type": "IS_A",
      "inherit_claims": false,
      "source_ref": "YOBI reviewed demo taxonomy: dish_yukgaejang lineage v2"
    }
  ],
  "source_type": "SYNTHETIC_WIKI",
  "source_refs": [
    "YOBI reviewed demo culinary summary: dish_yukgaejang v2"
  ],
  "license_state": "SYNTHETIC",
  "review_status": "REVIEWED_DEMO",
  "is_synthetic": true,
  "version": "external-demo-wiki-2026.08.17-v2",
  "updated_at": "2026-08-17"
}
---
# Yukgaejang

Yukgaejang is a Korean red soup traditionally associated with shredded beef, vegetables and aromatic seasoning simmered together.

It is served steaming hot with a savoury, robust and commonly spicy profile. The menu must confirm protein and actual heat.

This is general culinary guidance, not a statement about one merchant's recipe, ingredients, certification, allergens, portion or menu-specific spice level. Check the current merchant information before ordering.
