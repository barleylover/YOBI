...
readonly RUN_ID="diagnostic-four-${RELEASE_ID}"
readonly REMOTE_OUTPUT_DIR="/opt/yobi/shared/evidence/recommendation-v2"
readonly REMOTE_ARTIFACT="${REMOTE_OUTPUT_DIR}/diagnostic-four-${RUN_ID}.json"
...
[[ "${YOBI_GUARDED_SSH_WINDOW:-}" == "1" \
  ...
  || { printf 'Diagnostic four requires the guarded LB SSH transport.\n' >&2; exit 1; }
[[ ! -e "$LOCAL_ARTIFACT" && ! -e "$LOCAL_SIDECAR" ]] \
  || { printf 'Local diagnostic evidence target already exists.\n' >&2; exit 1; }
...
ssh "${ssh_options[@]}" "$SSH_USER@$HOST" \
  "sudo -n env PYTHONPATH='${release_path}/backend:${release_path}' \
    ...
    '${release_path}/venv/bin/python' \
    '${release_path}/scripts/recommendation_v2_live_harness.py' diagnostic-four \
...
[[ "$(cat "$LOCAL_SIDECAR")" == "$expected_sidecar" ]] \
  || { printf 'Diagnostic evidence checksum does not match the remote sidecar.\n' >&2; exit 1; }

if [[ "$harness_status" -ne 0 ]]; then
  printf 'Four-call diagnostic failed; release state is unchanged. evidence=%s\n' \
    "$LOCAL_ARTIFACT" >&2
  exit "$harness_status"
fi
printf 'Four-call diagnostic passed; release state is unchanged. evidence=%s\n' \
  "$LOCAL_ARTIFACT"
