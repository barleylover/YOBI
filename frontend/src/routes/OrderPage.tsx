  ...
  return (
    <main className="order-complete-shell">
      <section className="order-complete-card">
        ...
        <p className="eyebrow">{journeyCopy.mockConfirmed}</p><h1>{copy.orderConfirmed}</h1><p className="order-id">{orderId}</p>
        <div className="arrival-card"><Clock3 size={22} /><div><strong>{copy.arrival}</strong><span>{journeyCopy.etaDemo}</span></div></div>
        <div className="arrival-card"><Hotel size={22} /><div><strong>{copy.handoff}</strong><span>{journeyCopy.frontDeskHandoff}</span></div></div>
        <p className="demo-notice">{journeyCopy.paymentSuccess}</p>
        <div className="button-row"><Link className="primary-button" to="/"><RotateCcw size={18} /> {copy.anotherOrder}</Link><Link className="secondary-button" to="/"><MessageCircle size={18} /> {copy.conversation}</Link></div>
      </section>
    </main>
  );
}
